...
- [x] 3. P1 定员、领取与满员（后端）
  ...
  - [x] 3.1 按 inventory 生成 device/port，端口编号 `R1/01`、`S3/01`
    - 教师预置路由器口 IP 写入 port
  - [x] 3.2 实现 `POST /api/v1/classrooms/join`（仅 `client_kind`，无 join_code）
  ...
  - [x] 3.3 实现 `POST /api/v1/classrooms/{id}/open-claim`
    - 对已 waiting 连接立即随机发放或 `claim.full` 后结束连接
  - [x] 3.4 独立端 join 接受 `nic_mac`；托管端生成真实格式单播 MAC
    - 对应 R2 AC6 AC7
  - [x] 3.5 断线不释放已领角色；同 `connection_id` 重连恢复原 `device_id`
    - 仅教师结束课堂后角色池释放；教师机重启清空领取绑定
  - [x] 3.6 编写 P1 功能测试（需求覆盖）
...
- [x] 4. 检查点 P1 - 确保所有测试通过,如有疑问请询问用户
 ...
 - [x] 8. 检查点 P3 - 确保所有测试通过,如有疑问请询问用户
  - 验收：`docs/04` §10.2 四条全绿；接口验收第 4、7 条

 - [x] 9. P4 模拟模式、选口、改 MAC 与 TAP 记帧（后端）
  - 依据：SRS §7、接口 §6 §7、R10–R15；正确性 7、8、9
  - 阶段基线：`docs/04` §11.2 四条自动化通过；正确转发后本机 `frame.departed` 清空展示
  - [x] 9.1 实现模式切换 `POST /api/v1/classrooms/{id}/mode` 并广播 `mode.changed`
  - [x] 9.2 实现 PC 组帧 `sim/send`：按 ARP 填 MAC，汉字原文，推送 `frame.built` 并投递下一跳
  - [x] 9.3 实现交换机/路由器 `forward`
    - 交换机按目的 MAC 对口；路由器按目的 IP 对口
    - 错口 `WRONG_PORT`，`at_device_id` 不变
    - 路由器对口后改写 dst_mac/src_mac，IP 与汉字不变
  - [x] 9.4 实现 TAP 自动转发与 `tap_log`、`frame.logged`
  - [x] 9.5 支持多 `frame_id` 并行；目的 PC `frame.arrived` 可投递；回复走同一套组帧逐跳
  - [x] 9.6 编写 P4 功能测试（需求覆盖）
...
- [x] 10. 检查点 P4 - 确保所有测试通过,如有疑问请询问用户
  - 验收：§11.2 四条全绿；接口验收第 5、6 条

- [x] 11. P5 独立学生端协议、Nginx 与容量冒烟（后端）
  - 依据：SRS §8、R16 R17 R18、`docs/04` §12
  - 阶段基线：100 条 WS 同时在线且教师快照可开；10 路 chat 或 4 路模拟传帧时服务可响应；冒烟用临时目录
  - [x] 11.1 实现独立学生端协议：采集本机 NIC MAC、join、维持 WS
    - `crates/student`；一个进程一个角色；本阶段不拉 UI
  - [x] 11.2 编写 Nginx 样例配置：`/teacher/` `/student/` `/api/` `/ws`
  - [x] 11.3 编写 100 条 WS 心跳冒烟脚本（临时目录）
  - [x] 11.4 编写 10 路 chat 与 4 路模拟传帧的容量功能测试

- [x] 12. 检查点 P5 后端总体验收 - 确保所有测试通过,如有疑问请询问用户
